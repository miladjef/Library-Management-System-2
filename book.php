...
<div class="container">
            ...
            <div class="filter-row">
                    ...
                    <select name="category" class="filter-select">
                        ...
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= $cat['cid'] ?>" 
                                    <?= $category_filter == $cat['cid'] ? 'selected' : '' ?>>
                                <?= $cat['category_name'] ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="filter-group">
                    ...
                    <input type="text" name="author" placeholder="نام نویسنده" 
                           value="<?= htmlspecialchars($author_filter) ?>" class="filter-input">
                </div>
                
                <div class="filter-group">
                    <label>سال انتشار:</label>
                    <input type="number" name="year" placeholder="1400" min="1300" max="1410"
                           value="<?= htmlspecialchars($year_filter) ?>" class="filter-input">
                </div>
                
                <div class="filter-group checkbox-group">
                    <label class="checkbox-label">
                        <input type="checkbox" name="available_only" 
                               <?= $available_only ? 'checked' : '' ?> class="filter-checkbox">
    ...
    <div class="results-info">
        <span class="results-count">
            <?= number_format($total_books) ?> کتاب یافت شد
        </span>
        <?php if (!empty($search)): ?>
            <span class="search-term">برای: "<?= htmlspecialchars($search) ?>"</span>
        <?php endif; ?>
    </div>
            ...
            <?php foreach ($books as $book_item): ?>
                <div class="book-card">
                    <div class="book-image">
                        <img src="<?= IMG_PATH . $book_item['image'] ?>" 
                             alt="<?= htmlspecialchars($book_item['book_name']) ?>"
                    ...
                    <div class="book-info">
                        <h3 class="book-title">
                            <a href="book.php?id=<?= $book_item['bid'] ?>">
                        ...
                        <p class="book-author">
                            ...
                            <?= htmlspecialchars($book_item['author']) ?>
                        </p>
                        
                        <p class="book-category">
                            <i class="fas fa-tag"></i>
                            <?= htmlspecialchars($book_item['category_name']) ?>
                        </p>
                        
                        <p class="book-year">
                            <i class="fas fa-calendar"></i>
                            <?= $book_item['publish_year'] ?>
                        </p>
                        
                        <div class="book-stats">
                            <span class="stat">
                                <i class="fas fa-copy"></i>
                                <?= $book_item['book_quantity'] ?> نسخه
                            </span>
                            <span class="stat available-stat">
                                <i class="fas fa-check"></i>
                                <?= $book_item['available_count'] ?> موجود
                            </span>
                        </div>
                    </div>
                    
                    <div class="book-actions">
                        <a href="book.php?id=<?= $book_item['bid'] ?>" class="btn btn-outline btn-sm">
                            <i class="fas fa-eye"></i>
                            مشاهده جزئیات
                        </a>
                        
                        <?php if ($user_logged_in && $book_item['available_count'] > 0): ?>
                            <a href="reserve.php?book_id=<?= $book_item['bid'] ?>" class="btn btn-primary btn-sm">
    ...
    <?php if ($total_pages > 1): ?>
        <div class="pagination-wrapper">
            ...
            <div class="pagination-info">
                ...
                (<?= number_format($total_books) ?> کتاب)
            </div>
        </div>
    <?php endif; ?>
</div>
