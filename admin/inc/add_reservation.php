    ...
    <form action="#" method="POST" id="addReservationForm">
        ...
        <select name="mid" id="mid" required onchange="loadMemberInfo(this.value)">
            ...
            <?php foreach ($members as $m): ?>
                <option value="<?= $m['mid'] ?>" 
                        data-active="<?= $m['active_reservations'] ?? 0 ?>"
                        data-penalty="<?= $m['total_penalty'] ?? 0 ?>">
                    <?= htmlspecialchars($m['name'], ENT_QUOTES, 'UTF-8') ?> - 
                    کد: <?= htmlspecialchars($m['member_code'], ENT_QUOTES, 'UTF-8') ?>
                </option>
            <?php endforeach; ?>
        </select>
        ...
        <select name="bid" id="bid" required onchange="loadBookInfo(this.value)">
            ...
            <?php foreach ($availableBooks as $b): ?>
                <option value="<?= $b['bid'] ?>" 
                    ...
                    <?= htmlspecialchars($b['book_name'], ENT_QUOTES, 'UTF-8') ?> - 
                    نویسنده: <?= htmlspecialchars($b['author'], ENT_QUOTES, 'UTF-8') ?>
                    (موجود: <?= $b['available_quantity'] ?>)
                </option>
            <?php endforeach; ?>
        </select>
        ...
        <input type="text" 
               ...
               required>
        <small class="help-text">مهلت پیش‌فرض: <?= DEFAULT_LOAN_DAYS ?> روز</small>
