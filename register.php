...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['register'])) {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (empty($data['name']) || mb_strlen($data['name']) < 2) {
            $validation_errors[] = 'نام باید حداقل 2 کاراکتر باشد';
        }
        
        if (empty($data['surname']) || mb_strlen($data['surname']) < 2) {
            $validation_errors[] = 'نام خانوادگی باید حداقل 2 کاراکتر باشد';
        }
        
        // بررسی نام کاربری
        if (empty($data['username']) || strlen($data['username']) < 3) {
            $validation_errors[] = 'نام کاربری باید حداقل 3 کاراکتر باشد';
        ...
        if (!empty($data['phone']) && !preg_match('/^09\d{9}$/', $data['phone'])) {
            $validation_errors[] = 'شماره تلفن معتبر نیست (مثال: 09123456789)';
        }
        
        // بررسی رمز عبور
        if (strlen($data['password']) < 6) {
            $validation_errors[] = 'رمز عبور باید حداقل 6 کاراکتر باشد';
            ...
            <form method="POST" action="" class="auth-form" id="registerForm">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" 
                           ...
                           pattern="09\d{9}">
                    <small class="form-help">مثال: 09123456789</small>
                </div>
